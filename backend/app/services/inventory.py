...
def to_read(server: Server) -> ServerRead:
    return ServerRead(
        ...
        os_kind=server.os_kind or "linux",
        ...
        log_sources=_json_list(server.log_sources_json),
...
class InventoryService:
    ...
    def create_server(self, payload: ServerCreate, folder_id: int | None = None) -> ServerRead:
        ...
        server = Server(
            ...
            os_kind=(payload.os_kind or "linux"),
            jump_host=(payload.jump_host or "").strip(),
            jump_port=payload.jump_port or 22,
            jump_username=(payload.jump_username or "").strip(),
