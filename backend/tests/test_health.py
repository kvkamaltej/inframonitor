...
def test_startup_seeds_admin_and_no_demo_server() -> None:
    ...
    with engine.connect() as conn:
        ...
        # the default seed set: the three master lists, the RBAC menu matrix + its provisioning
        # marker, and the one-time guest-menu-additions marker. Assert the keys, not a bare count,
        # so adding a setting fails loudly and legibly rather than as "N != M".
        seeded = {row[0] for row in conn.execute(text("SELECT key FROM app_settings"))}
        assert seeded == {"application_types", "environments", "server_types", "role_menus", "role_menus_provisioned", "guest_menu_additions_v1"}
