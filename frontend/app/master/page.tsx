    ...
    <AppShell title="Master Data" subtitle="Manage reusable classifications">
      {({ me }) => me.role === "admin" ? (
        <section className="grid gap-4 px-6 py-6 md:grid-cols-2">
          ...
          <Link href="/master/ssh-configs" className="border border-line bg-panel p-5 dark:border-slate-700 dark:bg-slate-900"><div className="font-semibold">SSH Configs</div><p className="mt-2 text-sm text-slate-500">Reusable jump host / tunnel profiles, referenced from servers and database connections.</p></Link>
        </section>
